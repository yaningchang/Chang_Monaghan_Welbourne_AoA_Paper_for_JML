...
function out_result=examine_semantic_output_aoa(inputfile)
...
item_index=cellfun(@str2double, item)+1;
sub_semantic_vector=semantic_vector(item_index,:);
...
for m=1:1:size(s_out,1)
    ...
    dist_act_tar=sum((repmat(activation, size(s_out,1),1)-sub_semantic_vector).^2,2);
    ...
    out_result(m,:)=[item_index(m) test_word(m) nearest_tar(m) act_dist(m) correct(m)];
end
